...
var wordsData = words;
var dictionaryData = words;
var WORD_LENGTH = 5;
var FLIP_ANIMATION_DURATION = 500; // Set your animation duration here
var DANCE_ANIMATION_DURATION = 500; // Set your dance animation duration here
var guessGrid = document.querySelector("#guess-grid");
var tiles = Array.from(guessGrid.querySelectorAll(".tile"));
var alertContainer = document.querySelector(".alert-container");
var activeTilesIndices = [];
var currentRow = 0;
var keyboard = document.querySelector("[data-keyboard]");
var targetWord = getRandomWord(wordsData);
...
function pressKey(key) {
    var rowStart = currentRow * WORD_LENGTH;
    var rowEnd = rowStart + WORD_LENGTH;
    var rowTiles = tiles.slice(rowStart, rowEnd);
    var nextTile = rowTiles.find(function (tile) { return !tile.dataset.letter && !tile.dataset.locked; });
    if (!nextTile)
        return; // Row is full
...
function deleteKey() {
    ...
    var rowEnd = rowStart + WORD_LENGTH;
    var rowTiles = tiles.slice(rowStart, rowEnd);
    var activeTiles = rowTiles.filter(function (tile) { return tile.dataset.letter && !tile.dataset.locked; });
    var lastTile = activeTiles[activeTiles.length - 1];
    if (!lastTile)
        return; // No tiles to delete
...
function submitGuess() {
    var rowStart = currentRow * WORD_LENGTH;
    var rowTiles = tiles.slice(rowStart, rowStart + WORD_LENGTH);
    if (rowTiles.length !== WORD_LENGTH || rowTiles.some(function (tile) { return !tile.dataset.letter; })) {
        showAlert("Not enough letters");
        shakeTiles(rowTiles);
        return;
    }
    var guess = rowTiles.map(function (tile) { return tile.dataset.letter; }).join("");
...
function getActiveTiles() {
    updateActiveTiles();
    return activeTilesIndices.map(function (index) { return tiles[index]; });
}
...
function checkWinLose(guess, tiles) {
    ...
    var remainingTiles = guessGrid.querySelectorAll(":not([data-letter])");
