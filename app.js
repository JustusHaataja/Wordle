...
let isEnglish = false;
let wordsData = engWords;
let dictionaryData = engWords;
const WORD_LENGTH = 5;
...
if (languageToggle && languageLabel) {
    languageToggle.addEventListener("click", () => {
        ...
        const zKey = keyboard.querySelector("[data-key='Z']");
        const xKey = keyboard.querySelector("[data-key='X']");
        const öKey = keyboard.querySelector("[data-key='Ö']");
        const äKey = keyboard.querySelector("[data-key='Ä']");
        setTimeout(() => {
            if (isEnglish) {
                zKey.textContent = "Ö";
                xKey.textContent = "Ä";
